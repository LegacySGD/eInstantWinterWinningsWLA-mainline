			...
			<lxslt:component prefix="my-ext" functions="formatJson">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						var luckyNums = getLuckyNumbers(scenario);
						...
						// Output lucky numbers table.
						var r = [];
						r.push('<table border="0" cellpadding="2" cellspacing="1" width="100%" class="gameDetailsTable" style="table-layout:fixed">');
 						r.push('<tr><td class="tablehead" colspan="' + luckyNums.length + '">');
						...
						for(var i = 0; i < outcomeNums.length; ++i)
						{
							...
							r.push('<td class="tablebody" width="50%">');
 							if(checkMatch(luckyNums, outcomeNums[i]))
 							{
					...
					// Output: ["23", "9", "31"]
					function getLuckyNumbers(scenario)
					{
					...
					// Input: List of lucky numbers and the number to check
					// Output: true is number is contained within lucky numbers or false if not
					function checkMatch(luckyNums, boardNum)
					{
						for(var i = 0; i < luckyNums.length; ++i)
						{
							if(luckyNums[i] == boardNum || boardNum == "Z")
							{
								return true;
							}
						}
